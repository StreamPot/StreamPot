...
export async function setMetadata(metadata: JobMetadata) {
    await getClient().query(
        'INSERT INTO job_metadata (job_id, job_duration_ms) VALUES ($1, $2)',
        [metadata.job_id, metadata.job_duration_ms]
    );
    for (const asset of metadata.assets) {
        await getClient().query(
            'INSERT INTO asset_metadata (job_id, asset_id, size_bytes, type, name, ffprobe) VALUES ($1, $2, $3, $4, $5, $6)',
            [metadata.job_id, asset.id, asset.ffprobe.format?.size || 0, asset.type, asset.name || null, JSON.stringify(asset.ffprobe)]
        );
    }
}
